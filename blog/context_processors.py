...
def base_context(request):
    ...
    top_topics = models.Topic.objects.order_by('-blog_posts__count').annotate(Count('blog_posts')).values('id', 'name', 'slug', 'banner','blog_posts__count')
    all_topics = models.Topic.objects.order_by('-blog_posts__count').annotate(Count('blog_posts')).values('name', 'slug','banner')
